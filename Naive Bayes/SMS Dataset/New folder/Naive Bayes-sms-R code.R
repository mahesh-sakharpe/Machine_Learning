####### Naive bayes sms_raw dataset ######

library(tm)
library(textcat)

message <- read.csv(file.choose(),stringsAsFactors = F)
head(message)
...
windows()
wordcloud(cor_train,
          min.freq=3,max.words = 300,         
          random.order = F,rot.per = 0.25,colors = ifelse(message$type=="spam","red","blue"))
